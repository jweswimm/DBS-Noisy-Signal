function [KStateOut]= KMarkovChainFraction_GPE(V, KStateIn, t,dt)
...
Kstate = KStateIn;
  ...
  while (tswitch < (t+dt)) 
    ...
    rate(1) = 4.*an_gpe(V) * Kstate(1);
    rate(2) = rate(1) + 3.*an_gpe(V) * Kstate(2);
    rate(3) = rate(2) + 2.*an_gpe(V) * Kstate(3);
    rate(4) = rate(3) + 1.*an_gpe(V) * Kstate(4);
    rate(5) = rate(4) + 4.*bn_gpe(V) * Kstate(5);
    rate(6) = rate(5) + 3.*bn_gpe(V) * Kstate(4);
    rate(7) = rate(6) + 2.*bn_gpe(V) * Kstate(3);
    rate(8) = rate(7) + 1.*bn_gpe(V) * Kstate(2);
