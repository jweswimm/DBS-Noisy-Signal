function EI = Copy_of_BGnetworkWhile(pd,wstim,freq)
...
% addpath("H:\My Documents\NeuronProj");
addpath("gating");
...
vth=zeros(n,length(t_hold)); %thalamic membrane voltage
vsn=zeros(n,length(t_hold)); %STN membrane voltage
vge=zeros(n,length(t_hold)); %GPe membrane voltage
vgi=zeros(n,length(t_hold)); %GPi membrane voltage
S2=zeros(n,1); S21=zeros(n,1); S3=zeros(n,1); 
S31=zeros(n,1);S32=zeros(n,1); S4=zeros(n,1); 
Z2=zeros(n,1);Z4=zeros(n,1);
t = zeros(length(t_hold),1);

%%with or without dbs
Idbs=creatdbs2(freq,tmax,tmin); %creating DBS train with frequency freq 
%jww changed from createdbs(freq, tmax, tmin) -> createdbs(freq, tmax, dt)
if ~wstim; Idbs=zeros(1,length(t_hold)); end

%disp(Idbs(10000));
%Istim change --- Update with what joe says %Istim not related to dbs, jww

Istim = zeros(1,length(t_hold));
...
%initial conditions of stochastic variables
%First Focus on potassium channel
nk = 1000; %number of total k channels

%Set number of open/closed K channels at equilibrium
kno = 4; %4 open channel
knc = nk-kno; % 0 open channels
N_kopen(i)=kno;
N_kclosed(i)=knc;
...
while (t(i) < tmax)
    i = i+1;
    r=rand(2,1);
    lm = knc*an(vsn(1,i-1))+kno*bn(vsn(1,i-1)); %effective rate constant
    dt = 1/lm*log(1/r(1));
%     disp(dt);

    if (dt > dtmin)
        dt = dtmin; % impose a minimum step size with no change in No or Nc
    elseif (kno*bn(vsn(1,i-1)) > r(2)*lm) %channel closing
            kno = kno -1;
            knc = knc +1;
    else
        kno=kno+1;
    ...
    N_kopen(i)=kno;
    N_kclosed(i)=knc;
    gkstoch=gk(2)*kno/nk; %stochastic k+ conductance
    ...
    Il1=0;%gl(1)*(V1-El(1));
    Ina1=0;%gna(1)*(m1.^3).*H1.*(V1-Ena(1));
    Ik1=0;%gk(1)*((0.75*(1-H1)).^4).*(V1-Ek(1));
    It1=0;%gt(1)*(p1.^2).*R1.*(V1-Et);
    Igith=0;%1.4*gsyn(6)*(V1-Esyn(6)).*S4; 
    
    %STN cell currents
    Il2=gl(2)*(V2-El(2));
    Ik2=gkstoch*(N2.^4).*(V2-Ek(2)); %changed from gk(2) to gkstoch jww
    Ina2=gna(2)*(m2.^3).*H2.*(V2-Ena(2));
    It2=gt(2)*(a2.^3).*(b2.^2).*(V2-Eca(2));
    Ica2=gca(2)*(C2.^2).*(V2-Eca(2));
    Iahp2=gahp(2)*(V2-Ek(2)).*(CA2./(CA2+k1(2)));
    Igesn=0;%0.5*(gsyn(1)*(V2-Esyn(1)).*(S3+S31)); %Igesn=0; %jww commented out
    Iappstn=33-pd*10;
    ...
    %GPe cell currents
    Il3=0;%gl(3)*(V3-El(3));
    Ik3=0;%gk(3)*(N3.^4).*(V3-Ek(3));
    Ina3=0;%gna(3)*(m3.^3).*H3.*(V3-Ena(3));
    It3=0;%gt(3)*(a3.^3).*R3.*(V3-Eca(3));
    Ica3=0;%gca(3)*(s3.^2).*(V3-Eca(3));
    Iahp3=0;%gahp(3)*(V3-Ek(3)).*(CA3./(CA3+k1(3)));
    Isnge=0;%0.5*(gsyn(2)*(V3-Esyn(2)).*(S2+S21)); %Isnge=0;
    Igege=0;%0.5*(gsyn(3)*(V3-Esyn(3)).*(S31+S32)); %Igege=0;
    Iappgpe=0;%21-13*pd+r;

    %GPi cell currents
    Il4=0;%gl(3)*(V4-El(3));
    Ik4=0;%gk(3)*(N4.^4).*(V4-Ek(3));
    Ina4=0;%gna(3)*(m4.^3).*H4.*(V4-Ena(3));
    It4=0;%gt(3)*(a4.^3).*R4.*(V4-Eca(3));
    Ica4=0;%gca(3)*(s4.^2).*(V4-Eca(3));
    Iahp4=0;%gahp(3)*(V4-Ek(3)).*(CA4./(CA4+k1(3)));
    Isngi=0;%0.5*(gsyn(4)*(V4-Esyn(4)).*(S2+S21)); %Isngi=0;%special
    Igigi=0;%0.5*(gsyn(5)*(V4-Esyn(5)).*(S31+S32)); %Igigi=0;%special
    Iappgpi=0;%22-pd*6;
    
    %Differential Equations for cells
    %thalamic
    vth(:,i)= V1+dt*(1/Cm*(-Il1-Ik1-Ina1-It1-Igith));%+Istim(i))); %Istim commented out jww
    H1=H1+dt*((h1-H1)./th1);
    R1=R1+dt*((r1-R1)./tr1);
    ...
    %original
    vsn(:,i)=V2+dt*(1/Cm*(-Il2-Ik2-Ina2-It2-Ica2-Iahp2+Iappstn)); %removed the Igesn and Idbs, only kept STN equations
...
function an = an(v)
an = stn_ninf(v)/stn_taun(v);
end


function bn = bn(v)
bn = (1-stn_ninf(v))/stn_taun(v);
end
