function dbs=noisypulses(f,tmax,dt)
...
PinkNoise=dsp.ColoredNoise(1,width_counter*num_pulses+f,1);
WhiteNoise=dsp.ColoredNoise(0,width_counter*num_pulses+f,1);
pnoise=PinkNoise();
wnoise=WhiteNoise();


% disp(width_counter*num_pulses);
k=1;
%every pulse_increment make a pulse with some noise
for i = 1:num_pulses
    for j=0:width_counter %this loop just gives a square wave
        pink_pulse(pulse_increment*i+j)=amp+10*pnoise(k);
        white_pulse(pulse_increment*i+j)=amp+10*wnoise(k);
        disp(k);
        k=k+1;
    end
end
