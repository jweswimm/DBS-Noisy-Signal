function dbs=createnoisyDBS(f,tmax,dt)
...
PinkNoise=dsp.ColoredNoise(1,f,1);
WhiteNoise=dsp.ColoredNoise(0,f,1);
pnoise=PinkNoise();
wnoise=WhiteNoise();
...
for i = 1:num_pulses
    for j=0:width_counter %this loop just gives a square wave
        pink_pulse(pulse_increment*i+j)=amp+10*pnoise(i);
        white_pulse(pulse_increment*i+j)=amp+10*wnoise(i);
    end
end
